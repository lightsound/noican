...
- `noican fetch <id>` — model ids live in `crates/noican-models/src/manifest.rs`. `fastenhancer-b` is the smallest pinned-sha256 download (~450 KB, GitHub release). `fetch hush-48k` exercises `depends_on` (pulls the `hush` tar.gz into `models/hush/`).
