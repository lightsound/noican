...
|---|---|---|---|
| [Krasp](https://github.com/pilshchikov/krasp) | Swift 6.1 + Rust; **Hush**/DeepFilterNet; own `KraspHAL.driver`; menu bar; meters; strength control; GitHub Actions PKG builds | MIT | Useful for the app shell, the shared-memory ring contract with the driver, and the installer flow. **Not** a reference for the capture path, contrary to first impressions: it captures with `AVCaptureSession`, never creates an aggregate device, never touches an `AudioUnit`, and contains no Rust of its own — DeepFilterNet is `dlopen`ed as a prebuilt dylib. For AUHAL and aggregate devices the real references are `cpal` (Apache-2.0), `coreaudio-rs` (MIT/Apache-2.0), and `cubeb-coreaudio-rs` (ISC) |
...
| [mellonella](https://github.com/penta2himajin/mellonella) | Rust/Python PoC of speaker gating | Apache-2.0 | Design reference for the DIY gate (§6.2). Also the source of a useful empirical number: its worker-to-output ring had to grow from ~30 ms to ~60 ms because per-chunk ONNX inference jitter exceeds one callback period |
| [voce](https://github.com/espetro/voce) | Rust PoC of speaker gating | **AGPL-3.0** | Read-only. AGPL-3.0 is stricter than GPL and would force the whole app open; **no code, structure, or comment may be taken from it**. Its ring buffer stores the write index already reduced modulo the capacity, which makes overrun undetectable — use an absolute counter instead |
